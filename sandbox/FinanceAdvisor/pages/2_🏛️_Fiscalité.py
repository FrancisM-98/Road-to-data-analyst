...
# ─── Page Config ─────────────────────────────────────────────
st.set_page_config(page_title="Fiscalité · Finance Advisor", page_icon="🏛️", layout="wide")
...
with col1:
    revenu_brut = st.number_input("Revenu annuel brut (CHF)", 0, 1_000_000, 85_000, 1_000, key="rev_brut")
with col2:
    canton = st.selectbox("Canton", list(CANTONS_ROMANDS.keys()), key="canton_select")
with col3:
    communes_list = list(CANTONS_ROMANDS[canton].get("communes", {}).keys())
    commune = st.selectbox("Commune", ["(Moyenne)"] + communes_list, key="commune_select")

commune_val = None if commune == "(Moyenne)" else commune

col4, col5, col6 = st.columns(3)
with col4:
    marie = st.selectbox("Situation familiale", ["Célibataire", "Marié·e"], key="situation_fisc")
    is_marie = marie == "Marié·e"
with col5:
    enfants = st.number_input("Nombre d'enfants", 0, 10, 0, key="enfants_fisc")
with col6:
